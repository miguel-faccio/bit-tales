...
function createBoard() {
    ...
    gameBoard.style.gridTemplateColumns = 'repeat(20, 30px)'; // 20 colunas com 30px
    gameBoard.style.gridTemplateRows = 'repeat(20, 30px)'; // 20 linhas com 30px
    gameBoard.style.gap = '5px'; // Espaçamento entre os quadrados

    for (let y = 0; y < 20; y++) { // 20 linhas
        for (let x = 0; x < 20; x++) { // 20 colunas
...
function startGame() {
    ...
    treasureLocation = {
        x: Math.floor(Math.random() * 20), // Gera a posição X do tesouro
        y: Math.floor(Math.random() * 20)  // Gera a posição Y do tesouro
    };
    clicks = 0;
